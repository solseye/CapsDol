...
export default function AdminCalendarPage() {
  ...
  const handleApprove = async (id) => {
    if (
      window.confirm(
        "상담 신청을 승인하시겠습니까? (승인 시 1시간 30분 일정이 확보됩니다)",
      )
    ) {
      try {
        setLoading(true);
        await allowAdminReservation([{ id, approved: true, reason: null }]);
        setModalOpen(false);
        await fetchAdminCalendarByCurrentMonth();
      } catch (err) {
        alert("승인 처리에 실패했습니다.");
    ...
    if (window.confirm("정말 이 상담 신청을 불허(취소)하시겠습니까?")) {
      try {
        ...
        await disallowAdminReservation([
          { id, approved: false, reason: reason.trim() || null },
        ]);
        setModalOpen(false);
        await fetchAdminCalendarByCurrentMonth();
      } catch (err) {
                          ...
                          <>
                            {evt.status === "pending" && (
                              <button
                                className="adm-btn primary"
                                onClick={() =>
                                  handleApprove(evt.targetId || evt.id)
                                }
                              >
                                승인 확정
                              </button>
